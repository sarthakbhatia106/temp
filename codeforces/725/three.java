...
public class three {
    ...
    static void solve() {
        ...
        HashMap<Long, Integer> lastIdx = new HashMap<>();
        
        Arrays.sort(arr);

        for (int i = n - 1; i >= 0; i--) {
            if (!lastIdx.containsKey(arr[i])) {
                lastIdx.put(arr[i], i);
            }
            ...
            while (left < right) {
                if (arr[left] + arr[right] == sum) {
                    if (arr[left] == arr[right]) {
                        int num = lastIdx.get(arr[left]) - firstIdx.get(arr[right]) + 1;
                        int add = ((num - 1) * num) / 2;
                        ans += add;
                    } else {
                        int lCount = lastIdx.get(arr[left]) + 1 - left;
                        int rCount = right - firstIdx.get(arr[right]) + 1;
                        ans += (lCount * rCount);
                    }
                    left = lastIdx.get(arr[left]) + 1;
                    right = firstIdx.get(arr[right]) - 1;
